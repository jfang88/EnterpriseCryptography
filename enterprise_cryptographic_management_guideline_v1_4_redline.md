...
This section is the **primary algorithm catalogue** for developers, architects, reviewers, and auditors. It consolidates the current enterprise baseline, Chinese equivalents where relevant, PQC transition choices, and do-not-use guidance into one place so a reader can determine what to use now, what to plan for next, and what to avoid.

**How to use this section:**
- Use the **Recommended now** column for new implementations unless an approved interoperability or regulatory requirement applies.
- Use the **Chinese equivalent / option** column where mainland China interoperability, customer requirement, or regulatory context requires ShangMi or related Chinese commercial cryptography alignment.
- Use the **PQC choice / crypto-agility note** column to plan transition-ready designs now, even where the production baseline remains classical.
- Check [§3.3](#33-forbidden--deprecated-algorithm-list) before approving any design.

#### 3.2.1 Single-Reference Algorithm Catalogue

| Function Type / Use Case | Recommended now (enterprise baseline) | Typical Key Size / Parameters | Chinese equivalent / option | Typical Chinese Parameters | PQC choice / crypto-agility note | Do not use / avoid | Notes |
|--------------------------|----------------------------------------|-------------------------------|-----------------------------|----------------------------|----------------------------------|--------------------|-------|
| Symmetric encryption — bulk data at rest and in transit | AES-256-GCM | 256-bit key; 96-bit nonce; AEAD mandatory | SM4-GCM or SM4-CCM where China-specific interoperability is required | 128-bit key | No immediate PQC algorithm change required; symmetric crypto remains viable with strong key sizes | DES, 3DES, RC4, AES-ECB, unauthenticated CBC for new designs | Default to AES-256-GCM for global interoperability; use SM4 only when there is a defined China-specific need |
| Symmetric encryption — software-centric or non-AES-accelerated environments | ChaCha20-Poly1305 | 256-bit key | No direct ShangMi equivalent included in this policy | N/A | No immediate PQC algorithm change required | Legacy stream ciphers | Prefer AES-256-GCM when hardware acceleration and compliance requirements align |
| Asymmetric key exchange for TLS / mTLS / secure session setup | ECDHE P-384 or X25519 | P-384 curve or X25519 | SM2 key exchange in ShangMi profiles | SM2 curve / profile | Plan hybrid migration with ML-KEM-768; new designs SHOULD support crypto-agility and algorithm negotiation | RSA key transport, static DH, weak curves | Use standard TLS 1.3 profiles for global interoperability; use SM2-based profiles only where required |
| Public-key encryption / key wrapping for small payloads | RSA-4096 OAEP only where legacy interop requires it; otherwise prefer envelope encryption with KMS and DEKs | 4096-bit RSA with OAEP | SM2 public-key encryption | SM2 curve / profile | Prefer KEM-style architecture and prepare for ML-KEM-768 | RSA PKCS#1 v1.5 encryption, RSA < 2048 | For most new systems, do not directly encrypt business data with RSA; use envelope encryption |
| Digital signatures — application signing, JWT, certificates | ECDSA P-384 preferred; Ed25519 where ecosystem support is mature | P-384 or Ed25519 | SM2 signature with SM3 | SM2 + SM3 profile | Plan for ML-DSA-65; consider SLH-DSA for selected high-assurance or long-lived signature use cases | DSA, SHA-1 signatures, RSA PKCS#1 v1.5 signatures | SM2 is the closest Chinese role-equivalent to ECDSA but has different certificate and ecosystem rules |
| Code signing and release signing | ECDSA P-384 or Ed25519; HSM/KMS-backed keys only | P-384 or Ed25519 | SM2 signing where China-specific ecosystem requires it | SM2 + SM3 | Pilot hybrid classical + ML-DSA signing where long verification life is required | Exported private keys on developer laptops; SHA-1-signed releases | Long-lived artefacts should be prioritised for PQC transition planning |
| Hashing / integrity | SHA-256 minimum; SHA-384 preferred for long-lived assurance; SHA-3-256 for selected use cases | 256/384/512-bit output depending on use case | SM3 | 256-bit output | For long-lived designs, prefer stronger output sizes and document migration flexibility | MD5, SHA-1 | SM3 is the common Chinese counterpart to SHA-256 for integrity and signature support |
| Message authentication | HMAC-SHA256 or HMAC-SHA384 | 256-bit minimum key | HMAC-SM3 where required by ecosystem | 256-bit class deployment | No immediate PQC change required | HMAC-MD5, HMAC-SHA1 for new designs | Default to HMAC-SHA256 globally |
| Password storage | Argon2id; PBKDF2-SHA256 only in constrained FIPS or compatibility contexts | Argon2id with memory-hard parameters; PBKDF2 >= 600k iterations where used | No separate Chinese baseline defined in this policy | N/A | No immediate PQC change required | Unsalted hashes, reversible encryption, MD5, SHA-1-based password storage | Password storage is intentionally separated from SM-family adoption |
| Key derivation | HKDF-SHA256; HKDF-SHA384 for long-lived assurance; PBKDF2 only where specifically required | Context-specific | KDF constructions using SM3 where China-specific ecosystem requires it | Context-specific | Keep KDF use abstracted to support later algorithm replacement | Ad hoc home-grown derivation | Document salt, context, and separation-of-purpose rules |
| TLS server and client certificates | ECDSA P-384 certificates preferred; RSA-4096 only for legacy interop | P-384 or RSA-4096 | SM2 certificates with SM3 signature in ShangMi environments | SM2 + SM3 | Hybrid certificate pilots should be considered as standards mature; keep PKI design agile | SHA-1-signed certs; RSA < 2048 | Some deployments may require dual PKI or parallel certificate profiles |
| TLS protocol baseline | TLS 1.3 | Approved TLS 1.3 AEAD suites | RFC 8998 ShangMi TLS 1.3 suites using SM2, SM3, SM4 | ShangMi TLS 1.3 profiles | Track hybrid TLS and PQC handshake support as production-grade standards mature | SSLv3, TLS 1.0, TLS 1.1, weak TLS 1.2 suites | The protocol choice and the algorithm choice are linked; a China-specific TLS profile affects both |
| SSH host and user authentication | Ed25519 host keys; ECDSA P-384 acceptable | Ed25519 or P-384 | No mainstream ShangMi SSH profile included here | N/A | Track PQC SSH ecosystem maturity; keep host key rollout automatable | DSA, small RSA, weak MACs | Use global SSH standards unless a specific China-facing requirement exists |
| Long-term PQC planning baseline | N/A — use current approved classical algorithms with crypto-agility | N/A | No separate Chinese commercial PQC baseline adopted in this policy | N/A | ML-KEM-768 for key establishment, ML-DSA-65 for signatures, SLH-DSA for selected use cases; designs SHOULD store algorithm metadata and support future substitution | Classical-only long-term assumptions without migration planning | This policy uses NIST PQC standards as the enterprise transition baseline |

#### 3.2.2 Algorithm Selection Principles

- **CRYPTO-ALG-010** — New systems SHALL use the enterprise baseline algorithms in the table above unless an approved interoperability, customer, or regulatory requirement justifies a Chinese profile or legacy exception.
- **CRYPTO-ALG-011** — Where China-specific cryptography is required, the design SHALL document the specific interoperability driver, affected systems, and whether dual-stack or parallel PKI support is required.
- **CRYPTO-ALG-012** — New asymmetric designs SHALL include crypto-agility considerations, including algorithm metadata, rotation path, and PQC transition readiness.
- **CRYPTO-ALG-013** — Where long-lived confidentiality or signature verification is required, the design SHOULD identify the intended PQC migration path at design time.

#### 3.2.3 Crypto-Agility Statement

Developers and architects SHALL treat algorithm choice as a configurable design element rather than a hard-coded business decision. New designs SHOULD externalize algorithm identifiers, key IDs, key versions, and trust anchors so that a service can migrate from classical algorithms to hybrid or PQC algorithms without major application rewrites. See [§6.4](#64-crypto-agility-implementation-guidelines) and [§8.2](#82-cryptographic-function-design-template).
...
The inventory record SHALL contain enough detail to support operational management, audit, incident response, and PQC planning.
...
The inventory SHALL be maintained through a defined review cycle.
...
The authoritative algorithm selection catalogue is now [§3.2](#32-cryptographic-function-types). Section 3.2 consolidates enterprise baseline algorithms, Chinese equivalents, PQC options, key-size expectations, and do-not-use guidance so developers can work from a single reference point.

Appendix H remains available as a compact architecture and procurement crosswalk for comparing Western, Chinese, and European standards at a portfolio level.
...
- **[R25]** National People's Congress of the People's Republic of China. *Cryptography Law of the People's Republic of China.* <http://www.npc.gov.cn/englishnpc/c2759/c23934/202009/t20200929_384279.html>
- **[R26]** IETF / RFC Editor. *RFC 8998: ShangMi (SM) Cipher Suites for TLS 1.3.* <https://www.rfc-editor.org/rfc/rfc8998.html>
- **[R27]** BSI. *TR-02102-1 “Cryptographic Mechanisms: Recommendations and Key Lengths”.* <https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/Publications/TechGuidelines/TG02102/BSI-TR-02102-1.html?nn=916626>
- **[R28]** BSI. *TR-02102-2 “Cryptographic Mechanisms: Recommendations for Transport Layer Security (TLS)”.* <https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/Publications/TechGuidelines/TG02102/BSI-TR-02102-2.pdf>

---

*End of Enterprise Cryptographic Management Guideline v1.4 REDLINE DRAFT*
