...
- [5. Algorithm Approval Policy & Cipher Suite Standards](#5-algorithm-approval-policy--cipher-suite-standards)
  ...
  - [5.6 Cross-Standard Algorithm Tables](#56-cross-standard-algorithm-tables)
...
### 5.6 Cross-Standard Algorithm Tables

This section provides the main document summary of recommended algorithms and key sizes across Hong Kong, NIST/Western, Chinese, and European practice. It is intended to be used together with [§3.2](#32-cryptographic-function-types), [§3.3](#33-forbidden--deprecated-algorithm-list), [§6](#6-quantum-safe-migration-programme), and [Appendix H](#appendix-h--china--nist--europe-algorithm-comparison).

#### 5.6.1 How to Read These Tables

- **Recommended now** identifies the preferred enterprise baseline for new implementations.
- **Chinese equivalent** identifies ShangMi / commercial cryptography options used for China-focused interoperability or regulatory needs.
- **European note** identifies relevant European guidance used in this document, primarily BSI TR-02102.
- **PQC for consideration** identifies transition candidates to track or pilot now.
- **Do not use** identifies deprecated or forbidden choices for the relevant use case.

#### 5.6.2 Symmetric Encryption and AEAD

| Use Case | Recommended now (NIST / Western baseline) | Chinese equivalent / option | European note | PQC for consideration | Do not use | Difference / interoperability note |
|----------|--------------------------------------------|-----------------------------|---------------|-----------------------|------------|------------------------------------|
| Bulk data encryption at rest | AES-256-GCM | SM4-GCM or SM4-CCM where China-specific interoperability is required | BSI TR-02102 continues to recommend modern AEAD use and strong key lengths | No algorithm change required; symmetric crypto remains viable post-quantum with strong key sizes | DES, 3DES, RC4, AES-ECB | SM4 uses a 128-bit key while AES recommendation here is 256-bit; use SM4 only where required for Chinese compatibility |
| High-performance software encryption | ChaCha20-Poly1305 | No direct ShangMi stream-cipher equivalent used in this document | Allowed where platform support and threat model justify it | No immediate change | Legacy stream ciphers | Prefer AES-256-GCM where hardware acceleration and compliance profile permit |
| TLS 1.3 record protection | AES-256-GCM or ChaCha20-Poly1305 | SM4-GCM or SM4-CCM in RFC 8998 TLS 1.3 profiles | BSI TLS guidance recommends TLS 1.3 and modern AEAD suites | Hybrid key exchange rather than symmetric change | CBC-mode TLS suites | Chinese TLS profiles rely on SM4 together with SM2/SM3 in ShangMi suites |

#### 5.6.3 Key Exchange, Public-Key Encryption, and TLS Authentication

| Use Case | Recommended now (NIST / Western baseline) | Chinese equivalent / option | European note | PQC for consideration | Do not use | Difference / interoperability note |
|----------|--------------------------------------------|-----------------------------|---------------|-----------------------|------------|------------------------------------|
| TLS / mTLS key exchange | ECDHE over P-384 or X25519 | SM2 key exchange in ShangMi TLS profiles | BSI guidance prefers strong modern elliptic-curve mechanisms and limits classic-only use over time | ML-KEM-768 in hybrid mode with classical exchange | RSA key transport, static DH | Use Western suites for general interoperability; use SM2 suites when systems must interoperate with China-specific TLS profiles |
| Small-payload public-key encryption / key wrap | RSA-4096 OAEP only for legacy interop; otherwise prefer KEM-style design with ECDH + envelope encryption | SM2 public-key encryption | BSI trend is away from legacy RSA padding profiles toward modern mechanisms | ML-KEM-768 | RSA PKCS#1 v1.5, RSA < 2048 | SM2 is the nearest Chinese equivalent to ECC-based public-key protection, but ecosystem support is narrower outside China |
| Server certificate authentication | ECDSA P-384 preferred; Ed25519 where supported | SM2 with SM3 certificate profile | BSI recommends strong signatures and modern TLS profiles | ML-DSA in hybrid certificate / signing pilots | SHA-1-signed certs, RSA PKCS#1 v1.5 signatures | Cross-border services may need dual trust models if both public WebPKI and SM2 PKI are required |

#### 5.6.4 Signatures, Hashing, MAC, and Password Protection

| Use Case | Recommended now (NIST / Western baseline) | Chinese equivalent / option | European note | PQC for consideration | Do not use | Difference / interoperability note |
|----------|--------------------------------------------|-----------------------------|---------------|-----------------------|------------|------------------------------------|
| Digital signatures for applications | ECDSA P-384; Ed25519 where ecosystem support is mature | SM2 signature with SM3 | BSI TR-02102 gives longer-term guidance on strong signature mechanisms and key lengths | ML-DSA-65; SLH-DSA for selected high-assurance use cases | RSA PKCS#1 v1.5 signatures, DSA, SHA-1 signatures | SM2 roughly occupies the role that ECDSA occupies in Western stacks, but uses different curve/profile rules |
| Hashing / integrity | SHA-256 minimum; SHA-384 preferred for long-lived assurance; SHA-3 for selected use cases | SM3 | BSI and ENISA-style recommendations emphasise modern hashes and sufficient strength | Use larger-output hashes for long-term resilience where needed | MD5, SHA-1 | SM3 is the Chinese counterpart commonly compared to SHA-256 for integrity and signature support |
| Message authentication | HMAC-SHA256 or HMAC-SHA384 | HMAC-SM3 where ecosystem requires it | Strong keyed MACs remain acceptable | No immediate PQC change required | HMAC-MD5, HMAC-SHA1 for new designs | HMAC-SM3 may be used in China-specific stacks, but HMAC-SHA256 remains default for global interoperability |
| Password storage | Argon2id; PBKDF2-SHA256 only in constrained FIPS contexts | No mainstream Chinese equivalent standard included here for enterprise password storage | Use memory-hard password hashing where possible | No immediate PQC change required | Unsalted SHA-256, MD5, reversible encryption | This document keeps password storage separate from ShangMi algorithm adoption |

#### 5.6.5 Protocols, Legacy, and PQC Transition Summary

| Use Case | Recommended now | Chinese equivalent / option | European note | PQC for consideration | Do not use | Difference / interoperability note |
|----------|-----------------|-----------------------------|---------------|-----------------------|------------|------------------------------------|
| Web / API transport | TLS 1.3 | RFC 8998 TLS 1.3 ShangMi suites where required | BSI TLS guidance recommends TLS 1.3 and constrains legacy use | Hybrid TLS as standards mature | SSLv3, TLS 1.0, TLS 1.1 | Chinese TLS profile changes both handshake algorithms and record protection suite |
| SSH access | Ed25519 host keys; ECDSA P-384 acceptable | No mainstream ShangMi SSH profile included here | Follow strong key and MAC choices | Track PQC SSH ecosystem maturity | DSA, small RSA, weak MACs | Use global SSH standards unless a China-specific secure shell profile is contractually required |
| Long-term asymmetric protection | Strong classical curves and signatures for near-term deployments | SM2 for China interoperability | BSI indicates classic-only mechanisms have a limited horizon | ML-KEM-768 and ML-DSA-65 are the primary enterprise transition targets | Classical-only long-term protection assumptions beyond planned transition horizon | No broadly adopted public Chinese commercial standard is included here for enterprise PQC selection, so this document uses NIST PQC as the planning baseline |

A machine-readable version of these tables is provided in `algorithm_comparison_matrix_v1_2.csv` and the expanded narrative version is included in [Appendix H](#appendix-h--china--nist--europe-algorithm-comparison).
...
This appendix consolidates the summary tables in [§5.6](#56-cross-standard-algorithm-tables) and adds a compact comparison view for architecture and procurement reviews.
...
*End of Enterprise Cryptographic Management Guideline v1.3 REDLINE DRAFT*
